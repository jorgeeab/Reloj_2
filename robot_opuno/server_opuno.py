...
# -*- coding: utf-8 -*-
"""
Servidor Robot Opuno - Versión Reformulada y Compatible
=======================================================

Servidor Flask para el robot de riego/pump automatizado con:
...
try:
    ...
    # Intentar agregar el directorio padre al sys.path si se ejecuta como script dentro de robot_opuno
    try:
...
class RobotLogger:
    ...
    def __init__(self, capacity: int = 1000):
        ...
        try:
            self._py_logger = logging.getLogger('opuno_server')
            self._py_logger.setLevel(logging.INFO)
            log_path = str((LOGS_DIR / 'opuno_server.log').resolve())
...
ROBOT_PROFILES: Dict[str, Dict[str, Any]] = {
    "real": {
        "id": "real",
        "label": "Robot Opuno físico (Arduino)",
        "kind": "opuno",
        "is_virtual": False,
...
if VirtualRelojEnv is not None:
    ROBOT_PROFILES["virtual"] = {
        "id": "virtual",
        "label": "Robot Opuno virtual PyBullet",
        "kind": "opuno",
        "is_virtual": True,
...
def get_robot_status(force_fresh=False) -> RobotStatus:
    ...
    try:
        with env_lock:
            ...
            # Campos de conveniencia (uniformes)
            try:
...
def api_ui_registry():
    ...
    try:
        reg_path = (STATIC_DIR / "components" / "registry" / "opuno.json")
        if reg_path.exists():
...
TELEMETRY_INTERVAL = float(os.environ.get("OPUNO_TELEMETRY_INTERVAL", "0.2"))
            ...
            if isinstance(fl, dict):
                ...
                if usar_sens is None:
                    try:
                        ...
                        usar_sens = False
                # Extraer flujo objetivo (nuevo campo)
                f_tgt = 0.0
                try:
...
def initialize_system():
    ...
    logger.log("Iniciando Sistema Robot Opuno v1.0 (clon Reloj)")
...
def _start_visualizer() -> bool:
    ...
    try:
        ...
        candidates = [
            (BASE_DIR / "Robot Virtual" / "Robot Virtual" / "urdf" / "opuno_pump.urdf").resolve(),
...
if __name__ == "__main__":
    ...
    PORT = int(os.environ.get("OPUNO_PORT", "5007"))
    ...
    def open_browser():
        """Abre la cabina de Opuno en el navegador por defecto."""
        time.sleep(1.5)
