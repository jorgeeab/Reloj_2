...
(() => {
  ...
  function applyStatusSnapshot(s){
    ...
    try{
      ...
      // Evitar pisar el objetivo de flujo; el widget lo gestiona.
      const fwVolAct = document.getElementById('fw_volume_actual');
      if(fwVolAct){ fwVolAct.textContent = `${fmt(Number(s.volumen_ml||0),0)} ml`; }
      // Evitar pisar el objetivo de volumen; el widget lo gestiona.
      // El restante lo dibuja el widget.
    }catch{}
    ...
    try{
      if(window.RelojWidgets){
        window.RelojWidgets.broadcast('telemetry', {
          snapshot: s,
          flowActual,
          flowTarget: (s && s.caudalBombaMLs!=null) ? Number(s.caudalBombaMLs) : null,
