  ...
  function factory(ctx) {
    ...
    function refreshStats() {
      ...
      if (refs.volumeRemaining) {
        ...
        // Visual feedback
        if (rem != null) {
          if (rem <= 0.1 && state.targetVolume > 0) refs.volumeRemaining.style.color = '#10b981'; // Green when done
          else if (state.running) refs.volumeRemaining.style.color = '#38bdf8'; // Blue when running
          else refs.volumeRemaining.style.color = ''; // Default
        }
      }
    }
    ...
    const updateRunUI = (running, completed = false) => {
      ...
      // Logic: 
      // - If running: Show 'Detener' (Stop)
      // - If not running (completed or idle): Show 'Ejecutar' (Execute)
      btn.textContent = running ? 'Detener' : 'Ejecutar';
    ...
    function wireControls() {
      ...
      if (refs.reset) {
        refs.reset.addEventListener('click', async () => {
          // Reiniciar SOLAMENTE el volumen acumulado (el contador), manteniendo el objetivo
          // NO enviamos setpoints:0 ni flow:0
          const payload = {
            reset_volumen: 1,
            energies: { bomba: 0 } // Detener bomba por seguridad al resetear
          };
          try {
            await ctx.ControlChannel.send(payload);
            // Sincronizar estado UI: volumen actual a 0, pero MANTENER targetVolume
            state.lastVolumeActual = 0;
            // No tocamos state.targetVolume ni state.targetFlow

            updateRunUI(false, false);
            state.autoStopDone = false;
            state.completeHold = 0;
            refreshStats();
            try { ctx.debug && ctx.debug({ bomba: 'reset_volumen_keep_target' }); } catch { }
            ctx.toast && ctx.toast('Contador reiniciado');
          } catch {
            ctx.toast && ctx.toast('Error');
          }
        });
